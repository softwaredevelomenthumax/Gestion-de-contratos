...
router.post('/', auth, uploadContractWithGoogleDrive, async (req, res) => {
  try {
    console.log('📥 /contracts request received:', {
      ...
      hasGoogleDriveFiles: !!req.googleDriveFiles,
      googleDriveFilesCount: Array.isArray(req.googleDriveFiles) ? req.googleDriveFiles.length : 0,
      userId: req.user?.id,
      role: req.user?.role
    });
    ...
    // Calcular IVA usando el porcentaje enviado por el frontend (o 19% por defecto)
    const valorSinIVANum = parseFloat(valorSinIVA);
    const porcentajeIVA = parseFloat(porcentajeIVAStr) || 19;
    ...
    if (req.googleDriveFiles && req.googleDriveFiles.length > 0) {
      ...
      for (const fileData of req.googleDriveFiles) {
        try {
          // Use the category set by the middleware (based on the form field name)
          const fileCategoryMap = {
            'contrato': { category: 'contrato', type: 'Contrato' },
            'oferta':   { category: 'oferta',   type: 'Oferta' },
            'camara':   { category: 'camara',   type: 'Cámara' },
            'otros':    { category: 'otros',    type: 'Otros' },
          };
          const mapped = fileCategoryMap[fileData.category] || { category: fileData.category || 'contrato', type: 'Contrato' };
          const fileCategory = mapped.category;
          const fileType = mapped.type;
    ...
    try {
      ...
      const fallbackEmails = [
        process.env.CONTRACT_NOTIFICATION_FALLBACK_EMAIL,
        process.env.NOTIFICATION_FALLBACK_EMAIL,
        process.env.FROM_EMAIL
      ].filter(Boolean);
      const notificationEmails = getContractNotificationRecipients(notificationUsers, fallbackEmails);

      // 3. Enviar "Acción Requerida" a quienes deben responder
      if (notificationEmails.length > 0) {
        ...
        console.warn('⚠️ No se encontraron destinatarios válidos para notificación de contrato:', contract.id);
      }
